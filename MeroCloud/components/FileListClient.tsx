...
const FileListClient = ({ files }: { files: Models.Document[] }) => {
  ...
  const handleBulkTrash = async () => {
    ...
    await Promise.all(
      Array.from(selected).map((id) => {
        ...
        if (!file) return;
        return deleteFile({ fileId: id, bucketFileId: file.bucketFileId, path });
      }),
    );
    setSelected(new Set());
    setIsLoading(false);
      ...
      <section className="file-list">
        {files.map((file) => (
          <div key={file.$id} className="group relative">
            <div
              className={`absolute left-3 top-3 z-20 transition-opacity ${
            ...
            <div
              className={
                selected.has(file.$id) ? "rounded-[18px] ring-2 ring-brand" : ""
              }
            >
              <Card file={file} />
            </div>
